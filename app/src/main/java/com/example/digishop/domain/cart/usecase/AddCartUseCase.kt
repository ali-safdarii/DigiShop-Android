...
import com.example.digishop.utils.Constants.JWT_ERROR_MESSAGE
...
class AddCartUseCase @Inject constructor(
    ...
    suspend fun invoke(productId: Int,colorId: Int,finalPrice:Int,qty:Int ): AsyncResult<NoDataResponse> {
        try {
            ...
            val token = bearerTokenUseCase.bearerToken().first() ?: JWT_ERROR_MESSAGE

            val result =
                cartRepository.insert(productId = productId, colorId = colorId,finalPrice=finalPrice, qty = qty ,token = token)
